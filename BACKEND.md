...
| PATCH | `/profiles/me` | Bearer | `{ "name?", "avatar?", "email?" }` | `{ "id", "name", "email", "avatar", "updatedAt" }` |
| PATCH | `/profiles/me/password` | Bearer | `{ "currentPassword", "newPassword" }` — `newPassword` >= 6 ký tự | `{ "message": "Password updated successfully" }` |

Lỗi thường gặp cho đổi mật khẩu:
- `401 Unauthorized`: `currentPassword` không đúng.
- `400 Bad Request`: `newPassword` trùng với mật khẩu hiện tại.
