...
name: redux-rtk
description:
  Redux Toolkit and RTK Query patterns. Use when editing slice files, API
  definitions, store configuration, selectors, or any file importing from
  @reduxjs/toolkit.
---
...
export const selectActiveUsers = createSelector(selectUsersState, (users) =>
  users.ids.filter((id) => users.entities[id]?.status === "active"),
);
```
